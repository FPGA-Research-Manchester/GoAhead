...
namespace GoAhead.Commands.Debug
{
    ...
    class CheckExpectedPorts : NetlistContainerCommandWithFileOutput
    {
        class Expectation
        {
            ...
            {
                return this.InstanceNamePart + "." + this.ExpectedPortName 
                    + " InstanceName: " + (this.InstanceChecked ? "1" : "0") 
                    + " PortName: " + (this.PortNameChecked ? "1" : "0")
                    + " NetName:" + (this.NetNameChecked ? "1" : "0")
                    + " Index:" + (this.IndexChecked ? "1" : "0");
            }
        }
        ...
        protected override void DoCommandAction()
        {
            ...
            String line = "";
            TextReader tr = new StreamReader(this.ExpectFile);
            while ((line = tr.ReadLine()) != null)
            {
                ...
                expectation.ExpectedIndex = Int32.Parse(atoms[6]);
            ...
            Regex netNameMatch = new Regex(this.NetnameRegexp, RegexOptions.Compiled);
            ...
            foreach (Expectation expectation in expectations.Where(e => e.ExpectedIndex > 0))
            {
                ...
                foreach (XDLNet net in netlist.Nets)
                {
                    foreach (NetPin pin in net.NetPins.Where(np => np.GetDirection().Equals(expectation.Direction)))
                    {
                        ...
                        {
                            this.OutputManager.WriteWarning("Index of expectation " + expectation + " machted using GLOBAL_LOGIC net " + net.Name);
                            expectation.IndexChecked = true;
                            break;
                        }
                    }
            ...
            {
                this.OutputManager.WriteOutput("Could not satisfy expectation " + expectation);
            }
        }
