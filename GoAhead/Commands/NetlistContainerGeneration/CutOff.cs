    ...
    class CutOff : NetlistContainerCommand
    {
        ...
        private void CutOffISE()
        {
            ...
            foreach (XDLNet n in nlc.Nets)
            {
                ...
                n.Remove(p => this.Remove(p));
                n.RemoveAllPinStatements(np => this.Remove(np, nlc));
            ...
            nlc.Remove(inst => this.Remove(inst));
        ...
        {
            return FPGA.TileSelectionManager.Instance.IsSelected(FPGA.FPGA.Instance.GetTile(pip.Location).TileKey);
        }

        private bool Remove(TCLRoutingTreeNode node)
        {
            return FPGA.TileSelectionManager.Instance.IsSelected(node.Tile.TileKey);
        }

        private bool Remove(NetPin np, XDLContainer netlistContainer)
        {
            return FPGA.TileSelectionManager.Instance.IsSelected(FPGA.FPGA.Instance.GetTile(netlistContainer.GetInstance(np).Location).TileKey);
        }    

        private bool Remove(Instance inst)
        {           
            return FPGA.TileSelectionManager.Instance.IsSelected(FPGA.FPGA.Instance.GetTile(inst.Location).TileKey);
        }
