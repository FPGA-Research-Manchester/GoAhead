...
namespace GoAhead.Commands.Wizards
{
    public class BuildIslandStyleModule : Command
    {
        ...
        {
            String partialScript = this.ProjectDirectory + Path.DirectorySeparatorChar + "partial.goa";
            String extractScript = this.ProjectDirectory + Path.DirectorySeparatorChar + "extract_modules.goa";
            String loadModules = this.ProjectDirectory + Path.DirectorySeparatorChar + "load_modules.goa";
            ...
            printArchCmd.FileName = this.TopVHDLFile;
            printArchCmd.PartialAreaNames = this.PartialAreas;
            ...
            foreach (String partialArea in this.PartialAreas)
            {
                foreach (String vhdlModule in this.GetVHDLFilePaths(partialArea))
                {
            ...
            foreach (String vhdlModule in vhdlModules)
            {
                ...
                printCompDecl.FileName = this.TopVHDLFile;
            ...
            foreach (String partialArea in this.PartialAreas)
            {
                foreach (String vhdlModule in this.GetVHDLFilePaths(partialArea))
                {
                    ...
                    printSignalDecl.FileName = this.TopVHDLFile;
            ...
            foreach (String partialArea in this.PartialAreas)
            {
                foreach (String vhdlModule in this.GetVHDLFilePaths(partialArea))
                {
                    ...
                    printCondInst.FileName = this.TopVHDLFile;
            ...
            printConfigPackage.FileName = this.ProjectDirectory + Path.DirectorySeparatorChar + "config.vhd";
            printConfigPackage.PartialAreaNames = this.PartialAreas;
            printConfigPackage.ModulesPerArea = this.ModulesPerArea;
            ...
            {
                String partialAreaName = this.PartialAreas[i];

                // only consider those partial areas in which we build modules in
                if (!this.ModulesPerArea.Any(s => s.StartsWith(partialAreaName)))
                {
                    continue;
                }

                String moduleBlocker = this.ProjectDirectory + Path.DirectorySeparatorChar + partialAreaName + "_blocker.xdl";

                IslandCommandBuilder builder = new IslandCommandBuilder();
                builder.IslandName = this.PartialAreas[i];
                builder.VHDLWrapper = this.VHDLWrapper[i];
                builder.UCFFile = this.ProjectDirectory + Path.DirectorySeparatorChar + partialAreaName + ".ucf";
                ...
                foreach (String connectionPrimitives in this.ConnectionPrimitives)
                {
                ...
                if (this.ModulesPerArea.Any(s => s.StartsWith(partialAreaName)))
                {
                    ...
                    genBatchCmd.FileName = this.ProjectDirectory + Path.DirectorySeparatorChar + "create_bitstreams.bat";
                    genBatchCmd.Append = appendToBatchFile;
                    genBatchCmd.VHDLModules = this.ModulesPerArea.Where(s => s.StartsWith(partialAreaName)).ToList();
            ...
            extractScriptFile.WriteLine(Objects.Blackboard.Instance.LastLoadCommandForFPGA);

            for (int i = 0; i < this.PartialAreas.Count; i++)
            {
                foreach (String entityName in this.GetEntities(this.PartialAreas[i]))
                {
                    String moduleXDL = this.PartialAreas[i] + entityName + ".xdl";
                    String moduleBin = this.PartialAreas[i] + entityName + ".bin";

                    ClearSelection clearSel = new ClearSelection();
                    clearSel.Comment = "clear the current selection and select the module shape for module " + entityName + " in area " + this.PartialAreas[i];
                    extractScriptFile.WriteLine(clearSel.ToString());
                    foreach (Command sel in FPGA.TileSelectionManager.Instance.GetListOfAddToSelectionXYCommandsForUserSelection(this.PartialAreas[i]))
                    {
                        extractScriptFile.WriteLine(sel.ToString());
                    }

                    ExtractModule extractCmd = new ExtractModule();
                    extractCmd.Comment = "cut out the current selection and store the module " + entityName + " in area " + this.PartialAreas[i];
            ...
            loadScriptFile.WriteLine(Objects.Blackboard.Instance.LastLoadCommandForFPGA);
            ...
            for (int i = 0; i < this.PartialAreas.Count; i++)
            {
                ...
                storeCmd.UserSelectionType = this.PartialAreas[i];
            ...
            for (int i = 0; i < this.PartialAreas.Count; i++)
            {
                foreach (String entityName in this.GetEntities(this.PartialAreas[i]))
                {
                    ...
                    addCmd.FileName = this.PartialAreas[i] + entityName + ".bin";
